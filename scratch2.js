...
async function test() {
  ...
  for (const conv of data.data) {
     ...
     if (pData.participants && pData.participants.data) {
        const customer = pData.participants.data.find(p => p.id !== pageId);
        if (customer) {
            targetPsid = customer.id;
            targetName = customer.name;
            console.log(`Found a conversation ID: ${conv.id}`);
  ...
  if (!targetPsid) return console.log("Could not find a customer PSID.");
